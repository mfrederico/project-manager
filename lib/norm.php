...
class Norm
{
	...
	**/
    ...
    public function getOne($fromObj,$cols = '*',$getSet = 1)
    {
        $this->limit(0)->get($fromObj,$cols,$getSet);
        $idx = array_keys($this->results);
        $tmp = $this->results;
        $this->results[$idx[0]] = $tmp[$idx[0]][0];
        return($this);
    }
	...
	public function getObjsFrom($obj,$fromObj,$cols = '*',$getSet = 1)
	{
		...
		$Q="SELECT ".implode(',',$getCols)." FROM `{$joinTable}` INNER JOIN `{$this->prefix}{$objTable}` ON (`{$this->prefix}{$objTable}_id`=`{$joinTable}_{$objTable}_id`)";
	...
	public function get($fromObj,$cols = '*',$getSet = 1)
	{
		...
		$Q="SELECT ".implode(',',$getCols)." FROM {$this->prefix}{$tableName}";
		...
		if ($getSet)
		{
			...
			if (!empty($joins))
			{
				...
				foreach($joins as $joinTable=>$qrys)
				{
					foreach($qrys as $qry) 
					{
						if (!empty($this->constrainList)) 
							if (!in_array($qry['has'],$this->constrainList) && !in_array($qry['table'],$this->constrainlist)) unset($qry);

						if (!empty($qry))
						{
							$Q .= " LEFT JOIN {$this->prefix}{$qry['table']}_{$qry['has']} ON ({$qry['table']}_{$qry['has']}_{$qry['table']}_id={$qry['table']}_id) ";
							$Q .= " LEFT JOIN {$this->prefix}{$qry['has']} ON ({$qry['has']}_id={$qry['table']}_{$qry['has']}_{$qry['has']}_id) ";
						}
					}
				}
			}
	...
	*/
	...
	private function reduceTables($obj)
	{
		...
		if (!empty($this->maps[$table]))
		{
			...
			{
				//print "{$table}-&raquo;{$mapToTable} :: ".print_r($this->maps[$mapToTable],true)."<br />";
				$Q[$table][] = array('table'=>$table,'has'=>$mapToTable);
				/*
				if (isset($this->maps[$mapToTable])) 
				{
					$Q[$table][$mapToTable] = $this->reduceTables($mapToTable);
				}
				else $Q[$table][] = array('has'=>$mapToTable,'table'=>$table);
				*/
			}
			return($Q);
		}
	...
	private function getTableList()
	{
		if (empty($this->tableList))
		{
			...
			{
				if (!empty($this->prefix)) $val[0] = str_replace($this->prefix,'',$val[0]);
